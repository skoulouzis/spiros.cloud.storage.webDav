...
//			debug("Register: "+entry.getLRN());
	...
	private static Metadata getNodeMetadata(VNode vnode) throws VlException {
		...
		VAttribute att = vnode.getAttribute("modificationTime");
		m.setModifiedDate(att.getLongValue());
		m.setLength(vnode.getAttribute("length").getLongValue());
		return m;
	}

	private static List<AccessLocation> getAccessLocation(VNode vnode) throws VlURISyntaxException {
		List<AccessLocation> aLoc =new ArrayList<AccessLocation>();
		aLoc.add(new AccessLocation(new String[]{vnode.getVRL().toURIString()}));
		
		return aLoc;
	}

	private static List<ResourceEntry> nodes2Entries(String cloudLocationsUri,
			VNode[] nodes) throws VlURISyntaxException {
